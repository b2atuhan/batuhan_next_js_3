...
export default function App() {
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (nameRef.current && mailRef.current&&passAgainRef.current&&passRef.current) {
      alert('Name:' + nameRef.current.value+'\nEmail:' +mailRef.current.value+'\nPassword:' +passRef.current.value+'\nPassword Again:' +passAgainRef.current.value );
    }
  };
  ...
  return (
    ...
    <div className="flex flex-wrap flex-col bg-gray-300 m-20 rounded-3xl max-w-3xl self-center lg:min-h-[40vh] lg:min-w-[40vw] bg-opacity-60">
      ...
      <p className="text-black self-center font-bold lg:text-5xl md:text-4xl text-xl "> Sign Up</p>   
      </div>
      ...
      <form onSubmit={handleSubmit} style={{ margin: '20px',justifyContent: 'center' , display: 'flex', flexDirection:'column'}}>
        ...
        <label className={styles.responsiveLabel}>Name:</label>
        ...
        <label className={styles.responsiveLabel}>Password Again:</label>
        ...
        <input type="text" ref={nameRef} className={styles.responsiveInput}/>
        <input type="text" ref={mailRef} className={styles.responsiveInput}/>
        <input type="text" ref={passRef} className={styles.responsiveInput}/>
        <input type="text" ref={passAgainRef} className={styles.responsiveInput}/>            
        </div>
            </div>
        <button type="submit"
    ...
    <Link href="/signin" className=' bg-gray-600 rounded-lg p-2   text-xl'>
